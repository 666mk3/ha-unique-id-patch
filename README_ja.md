...
主に、**ONVIFカメラなどでMACアドレスが重複してしまい、「既に設定されています」となって登録できない問題**を解決するために作成しました。
...
このアドオンは、メモリ上の値を一時的に書き換えるのではなく、裏でHome Assistant Coreを停止させ(停止しないと書き換え反映しない)、設定ファイルを直接書き換えてから再起動するということをしています。
よってHome Assistantを再起動しても変更内容は維持され、元に戻ることはありません。
...
このアドオンはHome Assistantの内部設定ファイル (`/config/.storage/core.config_entries`) を直接書き換えます。
**コア設定ファイルの変更は非常にリスクが高く、ファイル破損やHome Assistantが起動しなくなる恐れがあります。**

安全のため、書き換えの直前に**常にタイムスタンプ付きのバックアップ**（例: `core.config_entries.bak_20260206_015000`）を同じ `.storage` フォルダ内に自動作成していますが、これはシステム全体のバックアップの代わりにはなりません。

**使用前に必ずHome Assistant標準のフルバックアップを取得してください。**
本アドオンの使用によるデータの破損やシステムの不具合について、作者は一切の責任を負いません。
